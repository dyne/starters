<script lang="ts">
	...
	const registerUser = async () => {
	  ...
	  credentialCreationOptions.publicKey.user.id = bufferDecode(credentialCreationOptions.publicKey.user.id);
		...
		const credential = await navigator.credentials.create({ publicKey: credentialCreationOptions.publicKey })
		if(credential != null) {

			const pkCred = credential as PublicKeyCredential
			let attestationObject = (pkCred.response as AuthenticatorAttestationResponse).attestationObject;
	...
	const loginUser = async () => {
		...
		const credential = await navigator.credentials.get({publicKey: credentialRequestOptions.publicKey})
...
<div class="p-4 space-y-4">
	Username:
  <br>
  <input bind:value="{username}" type="text" name="username" id="email" placeholder="i.e. foo@bar.com">
  <br>
  <br>
